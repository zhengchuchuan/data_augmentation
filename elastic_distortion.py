...
from utils.data_process import center_bbox_to_corner_bbox, labelme_json_to_bbox
...
for label_path in label_paths:
    ...
    for i in tqdm(range(random_times)):
        ...
        for _, label in enumerate(labelme_labels):
            ...
            # 弹性变换
            elastic_transformed_img = elastic_transform(labeled_patch_img, alpha=height*0.2, sigma=height*0.02, alpha_affine=height*0.02)
            # 生成随机变换矩阵
            ...
            perspective_transformed_img, transformed_bbox, transformed_mask = perspective_transform(elastic_transformed_img, mask, transform_matrix, label, )
